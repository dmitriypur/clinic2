...
- отключение акции филиала для врача: `doctor.extra.exclude_from_branch_promo_price`;
- взрослая акционная цена филиала: `branch.price`;
- детская акционная цена филиала: `branch.price_child`.

Текущее правило:

1. до 18 лет учитывается только `branch.price_child`, с 18 лет — только `branch.price`;
2. если соответствующая возрасту цена филиала заполнена, она имеет приоритет над ценой врача;
3. если такая цена филиала пуста, используется возрастная цена врача;
4. если `exclude_from_branch_promo_price` равен `true`, обе цены филиала игнорируются и показывается только цена врача;
5. если подходящей цены нет, цена не показывается.
...
Важный нюанс: branch promo price корректно применима только там, где известен филиал. В первом списке выбора врача филиал еще не выбран, поэтому там показывается цена врача. В schedule-шагах цена считается от текущего филиала и меняется при переключении филиала или даты, если дата меняет доступный филиал.
...
При закрытии виджета очищаются frontend-кэши, зависящие от локальных данных врачей и филиалов, и повышается их локальная версия. Поэтому после изменения цены врача, акционных цен филиала или флага исключения достаточно закрыть и повторно открыть виджет; жесткая перезагрузка страницы не требуется. Запоздавший ответ старого запроса сохраняется под прежней версией и повторно не используется. Кэши клиник, календарей и слотов сохраняются до окончания их TTL.
...
- `BookingSiteDoctorsService` кеширует локальные данные и видимость врачей на 60s;
...
- ключи кэшей, содержащих локальные данные врачей и филиалов, включают общую версию данных виджета;
- сохранение или удаление врача, а также изменение филиалов города или удаление города повышает версию, поэтому старые payload перестают использоваться сразу.
...
- помнить, что branch promo price доступен только когда известен филиал, и может быть отключен для конкретного врача.
