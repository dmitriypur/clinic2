...
- 2026-04-06: в `BookingWidgetV3` возрастная валидация на последнем шаге стала диапазонной. В [resources/js/components/BookingWidgetV3/utils/doctorAgeUtils.js](/Applications/MAMP/htdocs/zrenie.clinic-7/resources/js/components/BookingWidgetV3/utils/doctorAgeUtils.js) строка `Ведёт приём` теперь читается и из `doctor.receives`, и из `doctor.extra.receives`, после чего парсится по правилу `одно число = только нижняя граница`, `два числа = нижняя и верхняя граница`; это покрывает записи вида `с 3 до 17 лет`. В [resources/js/components/BookingWidgetV3/components/PatientFormStep.vue](/Applications/MAMP/htdocs/zrenie.clinic-7/resources/js/components/BookingWidgetV3/components/PatientFormStep.vue) дата рождения теперь блокирует запись не только при возрасте меньше `от`, но и при возрасте больше `до`, если верхняя граница указана.
...
- На шаге `resources/js/components/BookingWidgetV3/components/PatientFormStep.vue` дата рождения теперь валидируется против возрастного диапазона, распарсенного из `selectedDoctor.receives` с fallback на `selectedDoctor.extra.receives`: одно число в строке трактуется как `от`, два числа — как `от/до`; если пациент младше нижней границы или старше верхней, виджет не отправляет заявку и показывает inline-предупреждение у поля даты.
...
- 2026-04-06: `resources/js/components/BookingWidgetV3/utils/doctorAgeUtils.js` переведён с парсинга одного минимального возраста на простой диапазонный режим: одно число в `doctor.extra.receives` трактуется как нижняя граница, два числа — как нижняя и верхняя граница; `resources/js/components/BookingWidgetV3/components/PatientFormStep.vue` теперь блокирует запись и для пациентов старше верхней границы, если она задана.
