...
- Для контентных страниц доступен кастомный тип `BlockType::APPARATUS_TREATMENT`: в `BlockResource` он хранит две фиксированные секции `payload.sections`, где у каждой есть свой заголовок, rich-text и изображение через отдельную media collection.
  - `ServiceResource`
  - `CityResource`
    - секция `UTM Трекер` редактируется кастомным field `UtmTrackerManager`: четыре таба (`Основной`, `Архив`, `Источники`, `Телефоны`) работают поверх справочников и `city_utm_campaigns`, а сервис `UtmTrackerService` синхронизирует изменения обратно в legacy `utm_phones` для публичной логики; `Возобновить` создаёт новую запись кампании с новым `id`, не переиспользуя архивную.
  - `ReviewResource`
  - `CategoryResource`
  - `TagResource`
  - `PromotionResource`
