...
- После обогащения врачей локальными данными сайта `BookingWidgetV3.vue` применяет единый util sort: backend order-map остаётся первичным, а вторичный tie-breaker берётся из `age_min_months`.
- На шаге [resources/js/components/BookingWidgetV3/components/PatientFormStep.vue](/Applications/MAMP/htdocs/zrenie.clinic-7/resources/js/components/BookingWidgetV3/components/PatientFormStep.vue) дата рождения переводится в возраст в месяцах и валидируется против `age_min_months/age_max_months`; та же модель теперь используется и в старом `OnlineAppointmentForm`.
