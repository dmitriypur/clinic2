...
- 2026-05-04: для `BookingWidgetV3` добавлен универсальный launch context через data-атрибуты. Корневой [resources/js/app.js](/Applications/MAMP/htdocs/zrenie.clinic-7/resources/js/app.js) теперь читает `data-appointment-entry`, `data-doctor-id`, `data-branch-id` и `data-skip-birth-date` через [resources/js/utilities/bookingLaunchContext.js](/Applications/MAMP/htdocs/zrenie.clinic-7/resources/js/utilities/bookingLaunchContext.js), передаёт `launchContext` в [resources/views/layouts/app.blade.php](/Applications/MAMP/htdocs/zrenie.clinic-7/resources/views/layouts/app.blade.php), а [resources/js/components/BookingWidgetV3/BookingWidgetV3.vue](/Applications/MAMP/htdocs/zrenie.clinic-7/resources/js/components/BookingWidgetV3/BookingWidgetV3.vue) применяет его через существующие методы выбора врача/филиала. Обычные кнопки без data-атрибутов продолжают открывать первый шаг, legacy `bookingStartMode` сохранён, а `data-skip-birth-date="true"` для doctor-context делает дату рождения редактируемой на финальной форме.
- 2026-04-27: в блоке лицензий [resources/views/components/block/licenses.blade.php](/Applications/MAMP/htdocs/zrenie.clinic-7/resources/views/components/block/licenses.blade.php) статичная иллюстрация переведена на адаптивный `<picture>`: desktop использует `public/images/blocks/licenses.webp` с PNG fallback `licenses.png`, mobile — `licenses-m.webp` с fallback `licenses-m.png`. Это заменило прежний единый `images/korgy-lupa.webp` и даёт отдельный mobile-арт плюс fallback для браузеров без WebP.
- 2026-04-27: ускорены frontend-шаги полного деплоя. В [Envoy.blade.php](/Applications/MAMP/htdocs/zrenie.clinic-7/Envoy.blade.php) `npm ci` теперь использует постоянный cache `/home/forge/zrenie.clinic/.npm-cache`, `--prefer-offline`, `--no-audit`, `--no-fund` и отключённый update-notifier, чтобы не скачивать все tarball-зависимости заново и не тратить время на audit-вывод во время деплоя. В [vite.config.js](/Applications/MAMP/htdocs/zrenie.clinic-7/vite.config.js) отключён расчёт gzip-размера (`reportCompressedSize: false`) для production build, а в [tailwind.config.js](/Applications/MAMP/htdocs/zrenie.clinic-7/tailwind.config.js) убран дубль content-паттерна и исправлен путь `app/View`.
- 2026-04-27: исправлен повторный popup выбора города после ручного переключения на другой город при отличающемся GeoIP. В [app/Http/Middleware/SetCityMiddleware.php](/Applications/MAMP/htdocs/zrenie.clinic-7/app/Http/Middleware/SetCityMiddleware.php) при явном выборе через `force_city` теперь дополнительно ставится cookie `manual_city_override`; если она совпадает с текущей `selected_city`, GeoIP mismatch не кладёт `detected_city` в session и не провоцирует popup на каждом переходе. Это сохраняет ручной выбор пользователя до следующей ручной смены города и не меняет первый GeoIP-popup, remembered redirects, UTM-redirect и прямые городские URL. Добавлен unit-тест в [tests/Unit/SetCityMiddlewareTest.php](/Applications/MAMP/htdocs/zrenie.clinic-7/tests/Unit/SetCityMiddlewareTest.php).
- 2026-04-10: в баннерных сетках [resources/views/components/banner/banners-grid.blade.php](/Applications/MAMP/htdocs/zrenie.clinic-7/resources/views/components/banner/banners-grid.blade.php) и [resources/views/components/banner/banners-grid-k.blade.php](/Applications/MAMP/htdocs/zrenie.clinic-7/resources/views/components/banner/banners-grid-k.blade.php) отключён frontend-рендер через Spatie responsive `srcset` для изображений. Вместо `getResponsiveImage()` сетка теперь использует обычный `picture` с одним `source` для mobile-file и обычный `img src` для desktop-file, чтобы браузер не выбирал слишком маленькие auto-generated варианты (`352w`, `491w`) на мобильных экранах. В [app/Models/Block.php](/Applications/MAMP/htdocs/zrenie.clinic-7/app/Models/Block.php) добавлены helper-методы `getImageAltText()` и `getImageTitleText()`.
- 2026-04-10: branch enrichment для booking-потоков стал надёжнее матчить локальные филиалы не только по `external_id`/адресу/названию, но и по `phone`, `email`, `coordinates`. Это нужно, чтобы `price` и другие city branch overrides стабильно доезжали до `doctor-flow` даже в тех кейсах, где внешний booking API отдаёт другой `external_id` или текст адреса. Добавлен feature-тест в [tests/Feature/BookingDoctorsByDateApiTest.php](/Applications/MAMP/htdocs/zrenie.clinic-7/tests/Feature/BookingDoctorsByDateApiTest.php) на match по телефону для `branches-availability`.
- 2026-04-10: в doctor-flow `BookingWidgetV3` исправлен проп `selectedBranch` для шага [resources/js/components/BookingWidgetV3/components/DoctorScheduleStep.vue](/Applications/MAMP/htdocs/zrenie.clinic-7/resources/js/components/BookingWidgetV3/components/DoctorScheduleStep.vue). До фикса `doctor-schedule` получал только `selectedBranchId`, поэтому на шаге выбора даты и времени branch promo price не мог перебить `doctor.extra.price`, даже если филиал уже был выбран. Теперь [resources/js/components/BookingWidgetV3/BookingWidgetV3.vue](/Applications/MAMP/htdocs/zrenie.clinic-7/resources/js/components/BookingWidgetV3/BookingWidgetV3.vue) прокидывает полный `selectedBranch`, и doctor-flow использует то же правило цены, что `clinic-flow` и `date-flow`.
- 2026-04-10: для doctor cards в `BookingWidgetV3` введено единое правило цены. Локальный branch enrichment [app/Services/BookingBranchEnrichmentService.php](/Applications/MAMP/htdocs/zrenie.clinic-7/app/Services/BookingBranchEnrichmentService.php) теперь подмешивает `price` из `city.branches`, а frontend util [resources/js/utilities/doctorPrice.js](/Applications/MAMP/htdocs/zrenie.clinic-7/resources/js/utilities/doctorPrice.js) считает effective price без новых запросов: в первом списке `Выбрать врача` всегда показывается `doctor.extra.price`, а во всех branch-aware шагах (`doctor-schedule`, `clinic-schedule`, `date-flow`) branch `price` перебивает цену врача, если заполнен. Компоненты [resources/js/components/BookingWidgetV3/components/DoctorSelectStep.vue](/Applications/MAMP/htdocs/zrenie.clinic-7/resources/js/components/BookingWidgetV3/components/DoctorSelectStep.vue), [resources/js/components/BookingWidgetV3/components/DoctorScheduleStep.vue](/Applications/MAMP/htdocs/zrenie.clinic-7/resources/js/components/BookingWidgetV3/components/DoctorScheduleStep.vue) и [resources/js/components/BookingWidgetV3/components/ClinicScheduleStep.vue](/Applications/MAMP/htdocs/zrenie.clinic-7/resources/js/components/BookingWidgetV3/components/ClinicScheduleStep.vue) переведены на этот util. Feature-тесты обновлены на прокидывание branch `price` через `/api/booking/clinics/{clinic}/branches` и `/api/booking/doctors/{doctor}/branches-availability`.
- 2026-04-10: в doctor-flow `BookingWidgetV3` ускорен отклик слотов на смену даты и уточнено правило выбора дефолтного филиала. Теперь [resources/js/components/BookingWidgetV3/BookingWidgetV3.vue](/Applications/MAMP/htdocs/zrenie.clinic-7/resources/js/components/BookingWidgetV3/BookingWidgetV3.vue) при `select-date` сразу начинает `loadSlots()` по уже выбранному филиалу, а обновление `branches-availability` идёт параллельно. Если текущий филиал на новую дату всё ещё имеет слоты, второй запрос слотов не выполняется; если нет, doctor-flow автоматически переключается на `meta.default_branch_id` или на первый филиал с `has_available_slots = true`. Это убирает лишнюю задержку перед показом времени и делает стартовый выбранный филиал более релевантным для врача с несколькими филиалами.
- 2026-04-10: в doctor-flow `BookingWidgetV3` убрано мерцание списка филиалов при смене даты на шаге календаря. [resources/js/components/BookingWidgetV3/BookingWidgetV3.vue](/Applications/MAMP/htdocs/zrenie.clinic-7/resources/js/components/BookingWidgetV3/BookingWidgetV3.vue) больше не показывает skeleton и не пересоздаёт branch-card массив при каждом `select-date`, если филиалы врача уже были загружены; вместо этого date-specific ответ `branches-availability` мержится in-place в существующие объекты филиалов и обновляет только availability-поля (`has_available_slots`, `available_slots`, `first_available_time`). Это сохраняет тот же backend flow, но убирает визуальную перезагрузку адресов офтальмологии.
- 2026-04-10: doctor-flow `BookingWidgetV3` переведён с клиентского `N+1` по филиалам на новый локальный proxy route `GET /api/booking/doctors/{doctor}/branches-availability`. В [resources/js/components/BookingWidgetV3/BookingWidgetV3.vue](/Applications/MAMP/htdocs/zrenie.clinic-7/resources/js/components/BookingWidgetV3/BookingWidgetV3.vue) шаг `Выбрать врача -> календарь` больше не вызывает `getClinicDoctors` по каждому филиалу и не проверяет слоты по каждому филиалу до первого рендера. Вместо этого [resources/js/services/bookingApi.js](/Applications/MAMP/htdocs/zrenie.clinic-7/resources/js/services/bookingApi.js) ходит в новый local proxy [app/Http/Controllers/Api/BookingDoctorBranchesAvailabilityController.php](/Applications/MAMP/htdocs/zrenie.clinic-7/app/Http/Controllers/Api/BookingDoctorBranchesAvailabilityController.php), который проксирует внешний endpoint `adminzrenie-laravel`, обогащает филиалы локальными city overrides через [app/Services/BookingBranchEnrichmentService.php](/Applications/MAMP/htdocs/zrenie.clinic-7/app/Services/BookingBranchEnrichmentService.php) и возвращает `meta.default_branch_id`. В результате doctor-flow выбирает один рекомендуемый филиал и грузит слоты только для него. Добавлен feature-тест [tests/Feature/BookingDoctorsByDateApiTest.php](/Applications/MAMP/htdocs/zrenie.clinic-7/tests/Feature/BookingDoctorsByDateApiTest.php).
- 2026-04-10: в [resources/js/components/BookingWidgetV3/BookingWidgetV3.vue](/Applications/MAMP/htdocs/zrenie.clinic-7/resources/js/components/BookingWidgetV3/BookingWidgetV3.vue) исправлена ветка `date-flow` шага `Выбрать дату`: `dateFlowLastAvailableDate` больше не перетирается `null`/меньшей датой при переключении месяцев, а сохраняется как максимум по уже просмотренным месяцам. Благодаря этому новое поведение сообщений синхронизировано со старыми ветками `doctor/clinic`: если выбранная дата уже позже последней доступной даты расписания, показывается `Расписание на эту дату ещё не доступно`, а не дефолтное `Данный врач не принимает в выбранный день`.
- 2026-04-09: локальный proxy календаря для date-flow `BookingWidgetV3` в [app/Services/BookingDoctorsByDateService.php](/Applications/MAMP/htdocs/zrenie.clinic-7/app/Services/BookingDoctorsByDateService.php) переведён с fan-out по дневному `doctors-by-date` на новый внешний monthly endpoint `/api/v1/cities/{city}/doctors-by-date/calendar`. [app/Services/BookingWidgetApiService.php](/Applications/MAMP/htdocs/zrenie.clinic-7/app/Services/BookingWidgetApiService.php) получил отдельный метод `getDoctorsByDateCalendarAvailability()`, а [app/Services/BookingSiteDoctorsService.php](/Applications/MAMP/htdocs/zrenie.clinic-7/app/Services/BookingSiteDoctorsService.php) теперь умеет отдавать список UUID публично видимых врачей текущего города, чтобы proxy сохранял прежнюю семантику скрытых/непубличных специалистов. Обновлён feature-тест [tests/Feature/BookingDoctorsByDateApiTest.php](/Applications/MAMP/htdocs/zrenie.clinic-7/tests/Feature/BookingDoctorsByDateApiTest.php).
- 2026-04-09: команда миграции legacy age fields [app/Console/Commands/MigrateDoctorAgeFieldsCommand.php](/Applications/MAMP/htdocs/zrenie.clinic-7/app/Console/Commands/MigrateDoctorAgeFieldsCommand.php) теперь переносит не только распарсенные числа из `extra.receives` в `age_min_months/age_max_months`, но и сохраняет исходную legacy-строку целиком в `extra.receives_text`, чтобы после деплоя не потерять старый текст при миграции врачей. Добавлен feature-тест [tests/Feature/MigrateDoctorAgeFieldsCommandTest.php](/Applications/MAMP/htdocs/zrenie.clinic-7/tests/Feature/MigrateDoctorAgeFieldsCommandTest.php).
- 2026-04-09: date-flow `BookingWidgetV3` дополнительно “закален” после первого прохода реализации. В [resources/js/components/BookingWidgetV3/components/ClinicScheduleStep.vue](/Applications/MAMP/htdocs/zrenie.clinic-7/resources/js/components/BookingWidgetV3/components/ClinicScheduleStep.vue) mobile-сценарий `Выбрать дату` больше не сбрасывает внутренний экран обратно в календарь после перехода к списку врачей, а в [resources/js/components/BookingWidgetV3/BookingWidgetV3.vue](/Applications/MAMP/htdocs/zrenie.clinic-7/resources/js/components/BookingWidgetV3/BookingWidgetV3.vue) удалены временные `console.log` из подсветки календаря. В backend [app/Services/BookingDoctorsByDateService.php](/Applications/MAMP/htdocs/zrenie.clinic-7/app/Services/BookingDoctorsByDateService.php) branch payload для `doctors-by-date` теперь пробрасывает `branch external_id`, чтобы city overrides филиалов стабильно матчились по жёсткому идентификатору, а calendar-proxy строит map локальных врачей один раз на весь диапазон дат, а не заново на каждый день. Добавлены feature-тесты [tests/Feature/BookingDoctorsByDateApiTest.php](/Applications/MAMP/htdocs/zrenie.clinic-7/tests/Feature/BookingDoctorsByDateApiTest.php) на оба новых proxy route, а [tests/Unit/DoctorAgeTest.php](/Applications/MAMP/htdocs/zrenie.clinic-7/tests/Unit/DoctorAgeTest.php) синхронизирован с текущими дефолтными age-фразами.
- 2026-04-09: [resources/js/components/BookingWidgetV3/components/SuccessStep.vue](/Applications/MAMP/htdocs/zrenie.clinic-7/resources/js/components/BookingWidgetV3/components/SuccessStep.vue) переведён на новый success-layout по макету для desktop и mobile. Экран теперь показывает карточку врача с фото и специальностью, отдельные rounded-блоки адреса и даты/времени, блок ссылок "Добавить к себе в календарь" с локальными иконками из `public/images/calendar-icons/*` и Telegram-ссылку из `currentCity.social_links.telegram`, если она есть. Для этого [resources/js/components/BookingWidgetV3/BookingWidgetV3.vue](/Applications/MAMP/htdocs/zrenie.clinic-7/resources/js/components/BookingWidgetV3/BookingWidgetV3.vue) начал дополнительно пробрасывать в `SuccessStep` read-only prop `selectedDoctor`.
- 2026-04-09: в `BookingWidgetV3` добавлен третий сценарий `Выбрать дату`. Уже существующая кнопка в [resources/js/components/BookingWidgetV3/components/StartStep.vue](/Applications/MAMP/htdocs/zrenie.clinic-7/resources/js/components/BookingWidgetV3/components/StartStep.vue) теперь открывает flow `start -> birth-date -> date-select -> form -> success`, где шаг `date-select` переиспользует [resources/js/components/BookingWidgetV3/components/ClinicScheduleStep.vue](/Applications/MAMP/htdocs/zrenie.clinic-7/resources/js/components/BookingWidgetV3/components/ClinicScheduleStep.vue), но правая колонка строится из нового city-aware proxy `doctors-by-date`. Для этого добавлены локальные API endpoints [app/Http/Controllers/Api/BookingCityDoctorsByDateController.php](/Applications/MAMP/htdocs/zrenie.clinic-7/app/Http/Controllers/Api/BookingCityDoctorsByDateController.php) и [app/Http/Controllers/Api/BookingCityDoctorsByDateCalendarController.php](/Applications/MAMP/htdocs/zrenie.clinic-7/app/Http/Controllers/Api/BookingCityDoctorsByDateCalendarController.php), backend-сервисы [app/Services/BookingDoctorsByDateService.php](/Applications/MAMP/htdocs/zrenie.clinic-7/app/Services/BookingDoctorsByDateService.php) и [app/Services/BookingSiteDoctorsService.php](/Applications/MAMP/htdocs/zrenie.clinic-7/app/Services/BookingSiteDoctorsService.php), а [resources/js/services/bookingApi.js](/Applications/MAMP/htdocs/zrenie.clinic-7/resources/js/services/bookingApi.js) получил методы для doctor-cards на дату и помесячной подсветки календаря. В `date-flow` невыбранные карточки специалистов остаются нейтральными; оранжевым выделяется только текущая выбранная карточка, а на mobile внутренний порядок экранов инвертирован: шаг начинается с календаря/времени и только потом показывает список врачей. Финальный [resources/js/components/BookingWidgetV3/components/PatientFormStep.vue](/Applications/MAMP/htdocs/zrenie.clinic-7/resources/js/components/BookingWidgetV3/components/PatientFormStep.vue) теперь показывает дату рождения как read-only поле во всех ветках виджета.
- 2026-04-09: для `BookingWidgetV3` добавлен локальный [resources/js/components/BookingWidgetV3/components/BookingWidgetModal.vue](/Applications/MAMP/htdocs/zrenie.clinic-7/resources/js/components/BookingWidgetV3/components/BookingWidgetModal.vue), который использует существующие modal-примитивы (`Dialog`, `Backdrop`, `Scrollable`, `CloseButton`), но управляет собственным `layoutMode`. В [resources/js/components/BookingWidgetV3/BookingWidgetV3.vue](/Applications/MAMP/htdocs/zrenie.clinic-7/resources/js/components/BookingWidgetV3/BookingWidgetV3.vue) шаги `doctor-schedule` и `clinic-schedule` теперь получают `schedule`-режим без внутренних белых отступов, а остальные шаги сохраняют стандартные paddings. Общий [resources/js/components/Modal/Modal.vue](/Applications/MAMP/htdocs/zrenie.clinic-7/resources/js/components/Modal/Modal.vue) больше не содержит widget-specific API.
- 2026-04-09: шаг [resources/js/components/BookingWidgetV3/components/ClinicScheduleStep.vue](/Applications/MAMP/htdocs/zrenie.clinic-7/resources/js/components/BookingWidgetV3/components/ClinicScheduleStep.vue) переведён на новый карточный UI специалистов. Логика шага сохранена прежней, но визуально desktop теперь показывает календарь/время слева и список специалистов справа, а mobile повторяет паттерн [DoctorScheduleStep.vue](/Applications/MAMP/htdocs/zrenie.clinic-7/resources/js/components/BookingWidgetV3/components/DoctorScheduleStep.vue) с двумя внутренними экранами: сначала выбор специалиста, затем календарь и время. Для отображения адресной строки карточек [resources/js/components/BookingWidgetV3/BookingWidgetV3.vue](/Applications/MAMP/htdocs/zrenie.clinic-7/resources/js/components/BookingWidgetV3/BookingWidgetV3.vue) дополнительно пробрасывает `selectedBranch` в `ClinicScheduleStep`.
- 2026-04-09: в [resources/js/components/BookingWidgetV3/components/ClinicSelectStep.vue](/Applications/MAMP/htdocs/zrenie.clinic-7/resources/js/components/BookingWidgetV3/components/ClinicSelectStep.vue) и [resources/js/components/BookingWidgetV3/components/DoctorScheduleStep.vue](/Applications/MAMP/htdocs/zrenie.clinic-7/resources/js/components/BookingWidgetV3/components/DoctorScheduleStep.vue) исправлен путь к иконке метро для Vite dev-server. Статические файлы из `public/` теперь подключаются как `/images/...`, без префикса `/public`, поэтому изображение одинаково отображается и в `npm run dev`, и после production build.
- 2026-04-09: филиалы `BookingWidgetV3` переведены на backend-обогащение локальными данными города по `cities.branches[].external_id`. Новый endpoint [app/Http/Controllers/Api/BookingClinicBranchesController.php](/Applications/MAMP/htdocs/zrenie.clinic-7/app/Http/Controllers/Api/BookingClinicBranchesController.php) принимает явный `site_city_id`, чтобы не зависеть от `SetCityMiddleware` внутри `/api`, получает branch-list из booking API через [app/Services/BookingWidgetApiService.php](/Applications/MAMP/htdocs/zrenie.clinic-7/app/Services/BookingWidgetApiService.php) и подмешивает `address`, `metro` и `city` через [app/Services/BookingBranchEnrichmentService.php](/Applications/MAMP/htdocs/zrenie.clinic-7/app/Services/BookingBranchEnrichmentService.php). Во frontend [resources/js/services/bookingApi.js](/Applications/MAMP/htdocs/zrenie.clinic-7/resources/js/services/bookingApi.js) branch-запросы идут в локальный `/api/booking/clinics/{clinic}/branches`, а [ClinicSelectStep.vue](/Applications/MAMP/htdocs/zrenie.clinic-7/resources/js/components/BookingWidgetV3/components/ClinicSelectStep.vue), [DoctorScheduleStep.vue](/Applications/MAMP/htdocs/zrenie.clinic-7/resources/js/components/BookingWidgetV3/components/DoctorScheduleStep.vue) и [SuccessStep.vue](/Applications/MAMP/htdocs/zrenie.clinic-7/resources/js/components/BookingWidgetV3/components/SuccessStep.vue) теперь строят адресные строки из enriched branch-object без хардкода метро.
- 2026-04-08: добавлен статический [public/sw.js](/Applications/MAMP/htdocs/zrenie.clinic-7/public/sw.js) с noop-service-worker, который на `activate` удаляет Cache Storage и сам снимает регистрацию. Проект не использует service worker, но локально путь `/sw.js` раньше попадал в catch-all Laravel route и отдавал HTML вместо JS; из-за старой browser registration это могло подмешивать stale-кэш/старый визуал и ломать dev-поведение Vite.
- 2026-04-08: локальный Vite dev-server в [vite.config.js](/Applications/MAMP/htdocs/zrenie.clinic-7/vite.config.js) зафиксирован на `127.0.0.1:5173` с `strictPort: true` и явным `hmr.host/hmr.port`. Это убирает зависимость от `localhost -> ::1`, уменьшает риск падения HMR WebSocket из-за IPv6/hostname resolution и больше не даёт `npm run dev` тихо перепрыгивать на `5174+` при занятом порте.
- 2026-04-07: в [resources/js/components/BookingWidgetV3/BookingWidgetV3.vue](/Applications/MAMP/htdocs/zrenie.clinic-7/resources/js/components/BookingWidgetV3/BookingWidgetV3.vue) после стартового экрана добавлен обязательный шаг `birth-date` на отдельном компоненте [BirthDateStep.vue](/Applications/MAMP/htdocs/zrenie.clinic-7/resources/js/components/BookingWidgetV3/components/BirthDateStep.vue) с маской `ДД.ММ.ГГГГ` и root-state `patientBirthDateDisplay/patientBirthDateIso`. Теперь doctor-flow и clinic-flow всегда получают `birth_date` до показа врачей, используют его в вызовах `bookingApi.getDoctorsByCity()` / `getClinicDoctors()` и дополнительно локально фильтруют врачей по `age_min_months/age_max_months`. В [PatientFormStep.vue](/Applications/MAMP/htdocs/zrenie.clinic-7/resources/js/components/BookingWidgetV3/components/PatientFormStep.vue) дата рождения больше не является первичной точкой фильтрации списка: поле остаётся в финальной форме, но автозаполняется из шага `birth-date` и служит только submit-значением с fallback-валидацией.
- 2026-04-07: возрастные ограничения врачей переведены со свободного `extra.receives` на structured-модель в `Doctor.extra`: [app/Support/DoctorAge.php](/Applications/MAMP/htdocs/zrenie.clinic-7/app/Support/DoctorAge.php) теперь форматирует `age_min_months`, `age_max_months` и необязательный `receives_text`, а [app/Filament/Resources/DoctorResource.php](/Applications/MAMP/htdocs/zrenie.clinic-7/app/Filament/Resources/DoctorResource.php) хранит их через UI `число + единица` с сохранением в месяцах. Публичный вывод врачей, `BookingWidgetV3`, старый `OnlineAppointmentForm`, `/api/schedule` и `/api/booking/doctors` больше не зависят от runtime-парсинга строки `receives`; для существующих данных добавлена команда [app/Console/Commands/MigrateDoctorAgeFieldsCommand.php](/Applications/MAMP/htdocs/zrenie.clinic-7/app/Console/Commands/MigrateDoctorAgeFieldsCommand.php), которая переносит legacy `extra.receives` в новые поля.
...
- `Doctor`:
  ...
  - structured age fields в `extra`: `age_min_months`, `age_max_months`, `receives_text`;
...
- `App\Http\Middleware\SetCityMiddleware`:
  ...
  - ручной выбор через `force_city` ставит `manual_city_override`; пока она совпадает с `selected_city`, отличающийся GeoIP не открывает повторный popup подтверждения города;
...
- `BookingBranchEnrichmentService`
  - сращивает API-филиалы виджета с `cities.branches` по `external_id`;
  - сейчас подмешивает `address`, `metro` и `city`, оставляя API-данные fallback-ом для пустых локальных полей;
  - намеренно не зависит от `SetCityMiddleware` и получает явный локальный `City`.
...
- Локальный dev-server Vite закреплён на `127.0.0.1:5173`; если порт занят, `npm run dev` теперь падает сразу вместо silent fallback на `5174+`.
...
### BookingWidgetV3
- В `date-flow` ветки `BookingWidgetV3` последнее доступное расписание теперь копится как максимум по уже просмотренным месяцам. Поэтому при переходе на будущий месяц без опубликованного расписания UI показывает `Расписание на эту дату ещё не доступно`, а сообщение `Данный врач не принимает в выбранный день` остаётся только для дат внутри уже доступного горизонта расписания.
- В `resources/js/components/BookingWidgetV3/BookingWidgetV3.vue` flow теперь начинается как `start -> birth-date -> doctor|clinic`, а при `mode = doctor|clinic` стартовый шаг по-прежнему может быть пропущен, но шаг даты рождения остаётся обязательным.
- Управление стартовым шагом вынесено в родительский слой (`resources/js/app.js`): `showCallbackModal(...)` поддерживает legacy `options.bookingStartMode` и новый `launchContext`, который можно задать через `data-appointment-entry`, `data-doctor-id`, `data-branch-id`, `data-skip-birth-date`.
- Если `launchContext`/`bookingStartMode` не переданы, виджет открывается с первого экрана.
- Новый шаг [resources/js/components/BookingWidgetV3/components/BirthDateStep.vue](/Applications/MAMP/htdocs/zrenie.clinic-7/resources/js/components/BookingWidgetV3/components/BirthDateStep.vue) использует маску `ДД.ММ.ГГГГ`, валидируется через [resources/js/components/BookingWidgetV3/utils/birthDate.js](/Applications/MAMP/htdocs/zrenie.clinic-7/resources/js/components/BookingWidgetV3/utils/birthDate.js) и сохраняет в root-state сразу два значения: masked display и ISO `birth_date`.
...
- Branch-данные виджета больше не читаются фронтом напрямую из внешнего booking API: `resources/js/services/bookingApi.js` запрашивает локальный `/api/booking/clinics/{clinic}/branches` и получает уже enriched branch-object.
- Во всех текущих ветках виджета (`по врачу`, `по клинике/филиалу`) и во всех будущих сценариях branch-object должен проходить через этот backend endpoint, чтобы `address/metro` из админки применялись одинаково.
- Enriched branch-object теперь также содержит `city` текущего сайта, поэтому фронт может централизованно собирать адресные строки из `city + metro + address` без knowledge о мультигородной логике.
- Для branch-endpoint локальный город передаётся явно как `site_city_id`; это защищает мультигород от регресса, потому что `/api` не использует `SetCityMiddleware` и не должен угадывать текущий город по cookie или route.
...
- Общий frontend util [resources/js/utilities/doctorAge.js](/Applications/MAMP/htdocs/zrenie.clinic-7/resources/js/utilities/doctorAge.js) работает только со structured age fields и строит `receives_display`/диапазон без regex-парсинга свободного текста.
- После обогащения врачей локальными данными сайта `BookingWidgetV3.vue` использует `birth_date` из root-state и фильтрует врачей дважды: сначала через параметр `birth_date` во внешнем booking API, затем локально через `age_min_months/age_max_months`; backend order-map остаётся первичным, а вторичный tie-breaker берётся из `age_min_months`.
- На шаге [resources/js/components/BookingWidgetV3/components/PatientFormStep.vue](/Applications/MAMP/htdocs/zrenie.clinic-7/resources/js/components/BookingWidgetV3/components/PatientFormStep.vue) поле даты рождения теперь автозаполняется из шага `birth-date`, становится read-only и уходит в submit как зафиксированное значение выбранного сценария; финальная возрастная проверка больше не дублирует раннюю фильтрацию doctor-list.
...
- Основной сценарий:
  ...
  - `npm ci --prefer-offline --no-audit --no-fund` с постоянным cache `/home/forge/zrenie.clinic/.npm-cache`;
  - `npm run build` через Vite без расчёта gzip-размера чанков;
...
- Для порядка врачей в `BookingWidgetV3` backend/admin order-map остаётся первичным источником, transport-слой не содержит doctor-бизнес-логики, а публичный frontend добавляет вторичную сортировку по `age_min_months` и использует те же structured поля для клиентской возрастной валидации.
- В `BookingWidgetV3` список врачей больше нельзя считать независимым от даты рождения: root-state `patientBirthDateIso` обязателен до doctor-flow и clinic-flow, doctor list может быть пустым уже на раннем шаге, а любые новые ветки виджета должны использовать тот же birth-date state вместо локального хранения даты в финальной форме.
- Для branch-enrichment в `BookingWidgetV3` нельзя полагаться на implicit current city внутри `/api`: локальный city context должен передаваться явно (`site_city_id`), иначе multi-city AJAX-запросы могут незаметно скатиться в default city.
- После перехода на structured age fields runtime не должен снова зависеть от `extra.receives`; legacy-строка нужна только как источник для одноразовой команды `php artisan doctors:migrate-age-fields`.
- Принудительный старт веток `Выбрать врача`/`Выбрать клинику` включается только явным `launchContext` или legacy `bookingStartMode`; без этого виджет всегда стартует с первого шага.
...
- Для локального frontend dev/HMR проект должен использовать явный IPv4-host `127.0.0.1`; возврат к `localhost`/`::1` может снова сломать WebSocket в браузере и скрыть проблему за silent-переключением порта.
- Проект не использует PWA/service worker; путь `/sw.js` должен оставаться статическим noop-файлом из `public`, иначе catch-all Laravel route может вернуть туда HTML и старые browser registrations начнут ломать локальные ассеты и reload-поведение.

## Журнал изменений
- 2026-04-27: блок `LICENSES` теперь выводит иллюстрацию через `<picture>` с отдельными static assets для desktop/mobile и WebP/PNG fallback: `public/images/blocks/licenses.webp`, `licenses.png`, `licenses-m.webp`, `licenses-m.png`.
- 2026-04-10: `BANNERS_GRID` и `BANNERS_GRID_K` перестали рендерить Spatie responsive `srcset` на публичном сайте. Причина: браузер на mobile выбирал маленькие generated-файлы из `responsive-images/*___media_library_original_352_281.jpg` и аналогичные, хотя пользователю нужен был исходный mobile или desktop asset целиком. Теперь для этих двух шаблонов используется `picture` с одним mobile `source` и обычным `img src` без авто-выбора candidate widths.
- 2026-04-10: `date-flow` в `BookingWidgetV3.vue` перестал терять `lastAvailableDate` при переключении на месяцы без слотов. Теперь ветка `Выбрать дату` хранит максимум последней доступной даты по уже просмотренным месяцам и показывает то же сообщение `Расписание на эту дату ещё не доступно`, что и старые ветки `doctor-schedule` / `clinic-schedule`, когда пользователь уходит дальше доступного горизонта расписания.
- 2026-04-09: локальный proxy `/api/booking/cities/{city}/doctors-by-date/calendar` переключён на новый внешний monthly endpoint `/api/v1/cities/{city}/doctors-by-date/calendar`, а `BookingSiteDoctorsService` получил метод видимых UUID врачей города для сохранения текущей фильтрации скрытых/непубличных специалистов. Новый proxy по-прежнему возвращает тот же envelope `data[]` с полями `date`, `total_slots`, `available_slots`, `available_doctors`, `first_available_time`, но больше не собирает месяц через batch дневных запросов.
- 2026-04-09: укреплён новый `date-flow` `BookingWidgetV3`: mobile `ClinicScheduleStep` перестал самопроизвольно сбрасывать экран врача обратно в календарь, `BookingWidgetV3.vue` очищен от debug `console.log`, `BookingDoctorsByDateService` начал пробрасывать `branch external_id` для надёжного city branch-enrichment и переиспользует одну map локальных врачей на весь месячный calendar-range вместо повторного lookup на каждый день. Добавлены feature-тесты `BookingDoctorsByDateApiTest` на `/api/booking/cities/{city}/doctors-by-date` и `/api/booking/cities/{city}/doctors-by-date/calendar`, а `DoctorAgeTest` обновлён под текущие дефолтные age-фразы.
- 2026-04-09: филиалы `BookingWidgetV3` переведены на backend-обогащение по `cities.branches[].external_id`: добавлены `BookingBranchEnrichmentService`, локальный endpoint `/api/booking/clinics/{clinic}/branches` с обязательным `site_city_id`, переключение `resources/js/services/bookingApi.js` на этот endpoint, новый frontend util `branchUtils.js` для сборки строк `city + metro + address`, удаление хардкода метро из `DoctorScheduleStep.vue`, показ составного адреса филиала в `ClinicSelectStep.vue` и `SuccessStep.vue` и тесты `BookingBranchEnrichmentServiceTest` / `BookingWidgetSortingTest` на match по `external_id` и multi-city безопасность.
- 2026-04-08: добавлен `public/sw.js`, который очищает Cache Storage и снимает stale service-worker registration для текущего origin; причина — локальный `/sw.js` не существовал как статический файл и попадал в catch-all страницу Laravel, что могло держать в браузере старые кэши и визуал.
- 2026-04-08: в `vite.config.js` добавлен явный `server.host = 127.0.0.1`, `port = 5173`, `strictPort = true` и `hmr.host/hmr.port = 127.0.0.1:5173`, чтобы локальный `npm run dev` не зависел от IPv6 loopback `::1` и не маскировал конфликты порта переходом на `5174+`.
- 2026-04-07: в `BookingWidgetV3` добавлен обязательный шаг `birth-date` между стартом и doctor/clinic flow; root-state виджета теперь хранит `patientBirthDateDisplay` и `patientBirthDateIso`, doctor-flow/clinic-flow используют `birth_date` уже до показа врачей, список врачей дополнительно локально фильтруется по `age_min_months/age_max_months`, а финальная форма лишь автозаполняет и отправляет дату рождения без первичной ответственности за раннюю фильтрацию.
- 2026-04-07: врачи переведены на structured age fields (`age_min_months`, `age_max_months`, `receives_text`) с единым presenter `App\Support\DoctorAge`; админка хранит возраст в месяцах через UI `число + единица`, публичный вывод и оба виджета записи используют `receives_display`, а для переноса старого `extra.receives` добавлена команда `php artisan doctors:migrate-age-fields`.
...
- 2026-04-07: `resources/js/utilities/doctorAge.js` стал единым frontend-источником doctor age rules и display-текста; `BookingWidgetV3`, `OnlineAppointmentForm`, doctor cards и публичные Blade-компоненты больше не используют runtime-парсинг `extra.receives`, а работают от `age_min_months/age_max_months/receives_text`.
- 2026-03-24: doctor-flow `BookingWidgetV3` отрефакторен по слоям: в `resources/js/services/bookingApi.js` удалена doctor-сортировка, order-map вынесены в `resources/js/services/bookingOrdering.js`, а в `resources/js/components/BookingWidgetV3/utils/doctorUtils.js` собраны дедупликация, merge локальных doctor-данных и итоговая сортировка.
...
- 2026-03-23: в `resources/js/app.js` добавлен необязательный `bookingStartMode` для `showCallbackModal(...)` и проброс `bookingWidgetV3Mode` в layout; позже текущие врачебные триггеры переведены на универсальный `launchContext` через data-атрибуты, а `bookingStartMode` оставлен как legacy fallback.
