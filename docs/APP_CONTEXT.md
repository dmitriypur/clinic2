...
- На шаге [resources/js/components/BookingWidgetV3/components/PatientFormStep.vue](/Applications/MAMP/htdocs/zrenie.clinic-7/resources/js/components/BookingWidgetV3/components/PatientFormStep.vue) поле даты рождения теперь автозаполняется из шага `birth-date`, остаётся редактируемым и уходит в submit как текущее значение формы; fallback-проверка возраста против `age_min_months/age_max_months` сохранена только как защита от рассинхронизации после ручного редактирования финального поля.
