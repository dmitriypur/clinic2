...
- UTM tracker по городам нормализован в три таблицы:
  ...
  - `city_utm_sources` — справочник `utm_source` с дефолтным телефоном и флагом `open_booking_widget`;
  - `city_utm_mediums` — правила `utm_medium` c привязкой к source, телефону, `start_date`, `end_date` и флагом `open_booking_widget`;
...
- `UtmTrackerService`
  ...
  - синхронизирует справочник телефонов, sources и medium-правила, включая `open_booking_widget` для source-only и medium-ссылок;
