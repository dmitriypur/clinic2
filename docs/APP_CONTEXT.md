...
- 2026-04-09: для локальной проверки `BookingWidgetV3` без реальной отправки формы в [resources/js/components/BookingWidgetV3/BookingWidgetV3.vue](/Applications/MAMP/htdocs/zrenie.clinic-7/resources/js/components/BookingWidgetV3/BookingWidgetV3.vue) добавлен preview-режим по query-параметру `booking_preview=1`. В этом режиме финальный submit пропускает `bookingApi.createApplication()` и сразу переводит flow на success-экран, а [resources/js/components/BookingWidgetV3/components/PatientFormStep.vue](/Applications/MAMP/htdocs/zrenie.clinic-7/resources/js/components/BookingWidgetV3/components/PatientFormStep.vue) меняет текст кнопки на `Показать результат` / `Открываем экран...`. Обычный production-flow без параметра не меняется.
