...
### Автоматическое определение города (GeoIP)
При первом посещении сайта система пытается определить город пользователя по его IP-адресу, чтобы сразу предложить ему релевантную версию сайта.

#### 1. Сервис `GeoIpService`
- **Расположение:** `app/Services/GeoIpService.php`.
- **Назначение:** Сервис принимает IP-адрес, делает запрос к внешнему API (Sypex Geo) и получает название города. Затем он ищет в таблице `cities` активный город с таким же названием (поиск регистронезависимый).
- **Важно:** Сервис игнорирует локальные IP-адреса (`127.0.0.1`, `::1`).

#### 2. Middleware `SetCityMiddleware`
Логика определения города встроена в существующий middleware:
1.  Выполняется только если в URL **нет** префикса города (например, `site.ru/`, а не `site.ru/kirov/`).
2.  Проверяет наличие cookie `city_confirmed`. Если cookie есть, пользователь уже делал выбор, и автоопределение не требуется.
3.  Если условий выше нет, вызывается `GeoIpService`.
4.  Если сервис находит город и этот город **не является городом по умолчанию**, он сохраняется в сессию (`session(['detected_city' => $city])`) для последующей передачи на фронтенд.

#### 3. Конфигурация `TrustProxies`
Для корректной работы на продакшн-сервере, который находится за балансировщиком нагрузки или прокси (например, Cloudflare, Nginx), необходимо, чтобы Laravel "доверял" прокси-заголовкам для получения реального IP пользователя.
- **Файл:** `app/Http/Middleware/TrustProxies.php`
- **Настройка:** Свойство `$proxies` должно быть установлено в `'*'`:
  ```php
  protected $proxies = '*';
  ```

#### 4. Сопоставление названий городов
Для успешного определения крайне важно, чтобы названия городов в вашей таблице `cities` в колонке `name` **в точности** совпадали с названиями, которые возвращает Sypex Geo API на русском языке. Например, "Киров", а не "г. Киров" или "Кировская область".
...
Компонент `resources/js/components/CityConfirmationModal.vue` отвечает за отображение всплывающего окна при первом визите.

- **Активация:** Компонент становится видимым, если одновременно выполняются два условия:
    1.  В глобальном объекте `window.config` присутствует объект `detectedCity`.
    2.  У пользователя отсутствует cookie `city_confirmed`.
- **Логика работы:**
    1.  Пользователю показывается окно с вопросом "Ваш город {{ detectedCity.name }}?" и две кнопки: "Да, верно" и "Выбрать другой".
    2.  При нажатии на **"Да, верно"** происходит редирект на URL определенного города (например, `/kirov/`).
    3.  При нажатии на **"Выбрать другой"** в окне появляется список всех доступных городов для выбора.
    4.  После любого выбора (автоматического или ручного) в браузере пользователя устанавливается cookie `city_confirmed` сроком на 1 год, чтобы окно больше не появлялось.
