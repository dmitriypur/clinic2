    ...
    x-data="{
        ...
        setMediumNodeValue(sourceKey, mediumNode, value) {
            ...
            if (mediumNode) {
                ...
                mediumNode.key = nextMedium || '__empty_medium__'
            }
        ...
        ensureSourceRule(sourceNode) {
            ...
            if (! sourceNode.source.default_phone_key && ! sourceNode.source.open_booking_widget) {
                return null
            }
        ...
        async requestStopSource(sourceNode) {
            const row = this.ensureSourceRule(sourceNode)
                            ...
                            <div>
                                <div class="utm-tracker-grid utm-tracker-grid--tree utm-tracker-row border-b border-gray-100 px-3 py-2 dark:border-white/10">
                                    ...
                                    <div class="utm-tracker-actions">
                                        ...
                                        <button type="button" class="inline-flex h-8 w-8 items-center justify-center rounded-md border border-gray-400 text-gray-600 transition hover:bg-gray-50 dark:border-white/30 dark:text-gray-300 dark:hover:bg-white/5 disabled:cursor-not-allowed disabled:opacity-60" x-bind:class="sourceNode.sourceRule || sourceNode.source?.default_phone_key ? '' : 'invisible'" x-bind:disabled="isPersisting || ! (sourceNode.sourceRule || sourceNode.source?.default_phone_key)" x-on:click="requestStopSource(sourceNode)" title="Остановить" aria-label="Остановить"><svg class="h-4 w-4" viewBox="0 0 20 20" fill="#5f6c82" aria-hidden="true"><path d="M6 6.75A.75.75 0 0 1 6.75 6h6.5a.75.75 0 0 1 .75.75v6.5a.75.75 0 0 1-.75.75h-6.5A.75.75 0 0 1 6 13.25v-6.5Z" /></svg></button>
                                            ...
                                            <div x-show="isMediumExpanded('tracking', sourceNode.key, mediumNode.medium)">
                                                <template x-for="campaignRow in mediumNode.campaigns" :key="campaignRow.key">
                                                    <div class="utm-tracker-grid utm-tracker-grid--tree utm-tracker-row border-b border-gray-100 px-3 py-2 dark:border-white/10" x-bind:class="isDuplicateCampaignPhone(campaignRow) ? 'bg-rose-50 dark:bg-rose-500/10' : 'bg-white dark:bg-gray-900'">
                                                        ...
                                                        <div class="utm-tracker-actions"><button type="button" class="inline-flex h-8 w-8 items-center justify-center rounded-md border border-gray-200 text-gray-600 transition hover:bg-gray-50 dark:border-white/10 dark:text-gray-300 dark:hover:bg-white/5 disabled:cursor-not-allowed disabled:opacity-60" x-bind:disabled="isPersisting" x-on:click="requestStopCampaign(campaignRow.key)" title="Остановить" aria-label="Остановить"><svg class="h-4 w-4" viewBox="0 0 20 20" fill="#374151" aria-hidden="true"><path d="M6 6.75A.75.75 0 0 1 6.75 6h6.5a.75.75 0 0 1 .75.75v6.5a.75.75 0 0 1-.75.75h-6.5A.75.75 0 0 1 6 13.25v-6.5Z" /></svg></button><button type="button" class="inline-flex h-8 w-8 items-center justify-center rounded-md border border-rose-200 text-rose-600 transition hover:bg-rose-50 dark:border-rose-500/20 dark:text-rose-300 dark:hover:bg-rose-500/10 disabled:cursor-not-allowed disabled:opacity-60" x-bind:disabled="isPersisting" x-on:click="requestDeleteCampaignRow(campaignRow.key)" title="Удалить campaign" aria-label="Удалить campaign"><svg class="h-4 w-4" viewBox="0 0 20 20" fill="#dc2626" aria-hidden="true"><path fill-rule="evenodd" d="M8.75 2.5a1.75 1.75 0 0 0-1.75 1.75V5H4.75a.75.75 0 0 0 0 1.5h.443l.663 8.61A2.25 2.25 0 0 0 8.102 17.5h3.796a2.25 2.25 0 0 0 2.244-2.39l.663-8.61h.445a.75.75 0 0 0 0-1.5H13V4.25A1.75 1.75 0 0 0 11.25 2.5h-2.5ZM11.5 5V4.25a.25.25 0 0 0-.25-.25h-2.5a.25.25 0 0 0-.25.25V5h3Zm-2 3.25a.75.75 0 0 1 1.5 0v5a.75.75 0 0 1-1.5 0v-5Zm-2.5.75a.75.75 0 0 1 1.5 0v4.25a.75.75 0 0 1-1.5 0V9Zm5-.75a.75.75 0 0 1 .75.75v4.25a.75.75 0 0 1-1.5 0V9a.75.75 0 0 1 .75-.75Z" clip-rule="evenodd" /></svg></button></div>
                                                    </div>
                                                </template>
                                            </div>
                                        </div>
