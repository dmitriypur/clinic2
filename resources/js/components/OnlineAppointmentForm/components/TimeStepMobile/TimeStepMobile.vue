<template>
  <div v-if="activeToggler !== null" class="flex-1 py-8 md:px-5">
    <div class="flex flex-col md:grid grid-cols-2 gap-4">
      <div :class="['[&_.vc-container]:border-none px-4', activeToggler === 1 ? 'order-2' : 'order-1']">
        <v-date-picker
          trim-weeks
          is-expanded
      ...
      <div :class="['flex flex-col gap-6 h-full px-4 md:px-0', activeToggler === 1 ? 'order-1' : 'order-3']">
        <DoctorSelector
          :doctors="doctors"
      ...
      <div :class="['mt-4 px-4 md:px-0', activeToggler === 1 ? 'order-3' : 'order-2']">
        <TimeSlots
          :weekday="weekday"
          :timeSlots="time"
      ...
      <div v-if="selectedDate && selectedTime" class="mt-4 px-4 md:px-0 order-4">
        <button
...
<script>
import {DoctorCard, DoctorSelector, TimeSlots} from "../TimeStep/components";
